...
class InvoiceController extends Controller
{
    ...
    public function create()
    {
        ...
        $project_id = request()->get('project_id');
        if($project_id==0){ //create default project 
            $project_id=$this->creatOrGetprojectID($business_id);
        }
       ...
       $defalut_location=BusinessLocation::where('business_id', $business_id)->Active()->first();;
        ...
        return view('project::invoice.create')
            ->with(compact('customers', 'defalut_location','customer_groups','types','invoice_schemes', 'default_scheme', 'project', 'statuses', 'discount_types', 'taxes', 'tax_attributes', 'business_locations'));
    }
    public function creatOrGetprojectID($business_id){
        $project_defalut=\DB::table('pjt_projects')
        ->where('business_id',$business_id)
        ->where('is_default',1)->first();
        if($project_defalut){
            return $project_defalut->id;
        }
         ...
         $input=[];
            $input['settings'] = [
                   ...
                   // dd(json_encode($input['settings'],true));
        $project_defalut_id=\DB::table('pjt_projects')->insertGetId([
            'business_id'=>$business_id,
            'status'=>'in_progress',
            'lead_id'=>Auth()->user()->id,
            'name'=>'Services Invoices',
            'settings'=>json_encode($input['settings'],true),
            'created_by'=>Auth()->user()->id,
            'is_default'=>1,
            ]);
            return $project_defalut_id;
    }
    /**
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $input['transaction_date'] = $this->commonUtil->uf_date($request->input('transaction_date'),true);
            ...
            $transaction_options_row=[];
            foreach($options as $key=>$val){
                $measuer=\App\Measurement::where('business_id',$business_id)->where('key',$key)->first();
                
                if(!empty($measuer)){;
                    if($measuer->type=="text"){
                        $transaction_options_row['key']=$key;
                        $transaction_options_row['label']=$measuer->label;
                        $transaction_options_row['val']=$val;
                        $transaction_options_row['img']=null;
                    }else{
                        $row_options=json_decode( $measuer->options,true);
                        if(is_array($row_options)){ 
                            foreach($row_options as $r){
                                if($r['label']==$val || $r['image']==$val ){
                                    $transaction_options_row['key']=$key;
                                    $transaction_options_row['label']=$measuer->label;
                                    $transaction_options_row['val']=$r['label'];
                                    $transaction_options_row['img']=$r['image'];
                                }
                            }
                        }
                    }
                    $transaction_options[]=$transaction_options_row;
                }
            }
           // dd($transaction_options);
            $transaction = ProjectTransaction::create($input);
            $transaction->options = json_encode($transaction_options,true);;
            $transaction->save();

            $transaction->invoiceLines()->createMany($invoice_lines);
            $project=Project::find($transaction->pjt_project_id);
            DB::commit();
            $output = [
                'success' => true,
        ...
        if($project->is_default){
             return redirect()->to('/sells')->with('status', $output);
        }
