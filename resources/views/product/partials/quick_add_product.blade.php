    ...
    <div class="modal-body">
      <div class="row">
        ...
        <div class="col-sm-4 hide">
          ...
          <br>
            <label>
              {!! Form::checkbox('enable_stock', 0, true, ['class' => 'input-icheck', 'id' => 'enable_stock']); !!} <strong>@lang('product.manage_stock')</strong>
        ...
        <div class="col-sm-6">
            <div class="row">
                @php
                $gtypes=\DB::Table('g_types')->get();
                @endphp
                @foreach($gtypes as $row)
                <div class="col-sm-4">
                    <img width="60" src="/images/{{$row->img}}">
                     <div class="form-group">
                          <lable>{{$row->name}}</lable>
                            <input type="radio" name="g_type" value="{{$row->img}}">
                    </div>
                </div>
                @endforeach
               <div class="clearfix"></div>
                ...
                @php
                $bockets=\DB::Table('bocket_types')->get();
                @endphp
                @foreach($bockets as $row)
                <div class="col-sm-4">
                    <img width="60" src="/images/{{$row->img}}">
                     <div class="form-group">
                          <lable>{{$row->name}}</lable>
                            <input type="radio" name="g_bocket" value="{{$row->img}}">
                    </div>
                </div>
                @endforeach
                <div class="clearfix"></div>
                ...
                @php
                $neck_types=\DB::Table('neck_types')->get();
                @endphp
                @foreach($neck_types as $row)
                <div class="col-sm-4">
                    <img width="60" src="/images/{{$row->img}}">
                     <div class="form-group">
                          <lable>{{$row->name}}</lable>
                            <input type="radio" name="g_neck_type" value="{{$row->img}}">
                    </div>
                </div>
                @endforeach
                 @php
                $kbk_types=\DB::Table('kbk_types')->get();
                @endphp
                @foreach($kbk_types as $row)
                <div class="col-sm-4">
                    <img width="60" src="/images/{{$row->img}}">
                     <div class="form-group">
                          <lable>{{$row->name}}</lable>
                            <input type="radio" name="g_kbk_type" value="{{$row->img}}">
                    </div>
                </div>
                @endforeach
      ...
      <div class="row">
        <div class="form-group col-sm-11 col-sm-offset-1">
          @include('product.partials.single_product_form_part', ['profit_percent' => 0, 'quick_add' => true ])
        </div>
      </div>
      <div class="hide">
