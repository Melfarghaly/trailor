	...
	<div class="col-xs-12">
		...
		<table class="table table-responsive table-slim">
			...
			<tbody>
				@forelse($receipt_details->lines as $line)
					...
					<tr>
						<td>
                            ...
                            @endphp
                            @if(!empty($product->g_type))
                            <img width="60" src="/images/{{$product->g_type}}">
                            @endif
                            @if(!empty($product->g_bocket))
                            <img width="60" src="/images/{{$product->g_bocket}}">
                            @endif
                            @if(!empty($product->g_kbk_type))
                            <img width="60" src="/images/{{$product->g_kbk_type}}">
                            @endif
                            @if(!empty($product->g_neck_type))
                            <img width="60" src="/images/{{$product->g_neck_type}}">
                            @endif
                        </td>
						<td class="text-right">
                ...
                <?php 
			   $discount_total=0; $i=0; $qty_counter= 0 ; $all_tax=0; $total_lines_before_tax=0;
			   $lines=\DB::table('pjt_invoice_lines')->where('transaction_id',$receipt_details->transaction_id)->get(); 
             
               ?>
			
               @foreach($lines as $line)
			    <tr>
			        <td>{{$line->task}} <small>{{$line->description}}</small></td>
			        <td>{{$line->quantity + 0 }}
			            <?php $qty_counter+= $line->quantity  ?>
			        </td>
			        <td>{{$line->rate + 0 }}</td>
			        <td>{{($total_before_tax= $line->rate * $line->quantity) + 0 }}
			            <?php $total_lines_before_tax+=$total_before_tax ?>
			        </td>
			        <td>
			            @php 
			            $tax_rate=\DB::table('tax_rates')->where('id',$line->tax_rate_id)->first()->amount ?? 0 ;
			           
			            $tax_amount= (($line->rate * $tax_rate)/100 *  $line->quantity) + 0 ;
			             @endphp
			             {{@num_format($tax_amount)}}
			        <?php  $all_tax += $tax_amount; ?>
			        </td>
			        <td>{{@num_format($total_before_tax + $tax_amount)  }}</td>
			    </tr>
			    @endforeach
			</tbody>
		</table>
...
@php 
$trans_options=\DB::table('transactions')->where('id',$receipt_details->transaction_id)->select('options')->first();
